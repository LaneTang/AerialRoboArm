...
**基于 STM32F103、FreeRTOS、BLDC 闭环控制、AS5600 反馈与 ELRS 遥控输入的轻量型无人机折叠操作臂电控系统样机**
...
> AerialRoboArm 是一个经过实机验证的嵌入式机器人电控项目，面向轻量型无人机挂载折叠操作臂场景，完成了从 STM32 外设抽象、FreeRTOS 双频调度、BLDC 闭环控制、ELRS 遥控输入到板载串口 testbench 验证的完整电控链路。

- 最终仓库分支：**`main = demo_v6`**
- 运行时结构：**1 kHz 高频控制线程 + 50 Hz 低频逻辑 / testbench 线程**
- 控制链路：**AS5600 位置反馈 + 定点 PID + 电压 FOC + SVPWM 输出**
- 遥控输入：支持 **ELRS MANUAL 模式**，可将遥控输入映射为闭环电机目标值
- 调试方式：**115200 8N1 串口 testbench**
- 实机结果：在当前 12 V 原型供电与机械重力负载条件下，**45° 目标指令可稳定保持在约 30° 的力矩平衡位置**
...
| 串口调试 | 115200 8N1 |
| 闭环实机结果 | 45° 指令下，样机可在约 30° 位置形成稳定力矩平衡 |
| 主要限制 | 当前 12 V 原型供电与机械负载条件下，电机转矩裕量不足以完整保持 45° 目标位 |
...
项目重点不是构建通用机器人框架，而是在 STM32F103 资源受限平台上完成一套**可运行、可观测、可实机验证**的控制系统。系统集成了 STM32 外设抽象、FreeRTOS 双频调度、BLDC 电机控制、AS5600 位置反馈、ELRS 遥控语义映射，以及用于分阶段硬件验证的板载串口 testbench。

当前 `demo_v6` 固件为最终演示版本，仓库按照作品集工程案例进行整理，包含源码、架构图、实机演示材料与设计文档。
...
| 素材 | 说明 |
| --- | --- |
| [整机系统全览图](./Figures/DEMO/整机系统全览图.jpg) | 集成样机的整体实物视图 |
...
当前 `demo_v6` 固件是定稿的板载 `testbench/demo` 版本。它验证了以下电控主链路：
...
最终 demo 运行时采用双频 FreeRTOS 结构：

- **1 kHz 高频控制线程**：负责电机反馈、控制计算、FOC 输出与 PWM 更新
- **50 Hz 低频逻辑线程**：负责控制台输入、RC 处理、demo 状态转移、日志输出与 testbench 编排
