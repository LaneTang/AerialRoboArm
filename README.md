...
**A real-hardware embedded robotics control prototype for a lightweight UAV-mounted folding arm, built on STM32F103, FreeRTOS, BLDC position control, AS5600 feedback, and ELRS remote input.**

[中文版本](./README-zh.md)

## TL;DR

AerialRoboArm is a real-hardware embedded robotics project for a lightweight UAV-mounted folding arm. It implements a complete electrical-control chain from STM32 peripheral abstraction and FreeRTOS scheduling to BLDC closed-loop control, ELRS remote input, and on-board serial testbench validation.

- Final repository branch: **`main = demo_v6`**.
- Runtime architecture: **1 kHz motor-control loop + 50 Hz logic/testbench loop**.
- Control chain: **AS5600 feedback + fixed-point PID + voltage-FOC + SVPWM output**.
- Remote control: **ELRS MANUAL mode** maps RC input into closed-loop motor targets.
- Debug interface: **115200 8N1 serial testbench** for alignment, open-loop test, closed-loop test, RC control, AUTO step test, and telemetry.
- Resource usage on STM32F103C8T6: **RAM 20,104 B / 20 KB = 98.16%**, **FLASH 65,412 B / 64 KB = 99.81%**.
- Hardware validation: the prototype can execute and hold folding-arm closed-loop motion. Under the current 12 V prototype setup and mechanical gravity load, a 45° command reaches a stable torque-balance position around 30°, exposing the torque-margin limitation of the present hardware configuration.
...
The project focuses on embedded control under constrained MCU resources. It combines STM32F103 peripheral abstraction, FreeRTOS dual-rate scheduling, BLDC motor control, AS5600 position feedback, ELRS remote-control semantics, and an on-board serial testbench for staged hardware validation.

The current `demo_v6` firmware is a final demonstration version. It is organized as an engineering case study with real hardware evidence, curated architecture diagrams, demo media, and design documentation. It is not intended to be a general-purpose robotics framework.

## Key Results

| Item | Result |
| --- | --- |
| Final branch | `main = demo_v6` |
...
| Serial testbench | 115200 8N1 |
| Memory usage | RAM 20,104 B / 20 KB = 98.16%; FLASH 65,412 B / 64 KB = 99.81% |
| Closed-loop demo | 45° command reaches a stable torque-balance position around 30° under the current prototype load |
| Main limitation | Motor torque margin is insufficient for full 45° target holding under the present mechanical load and 12 V prototype setup |
...
Under the current prototype load, the 45° command settles at approximately 30° because the BLDC output torque reaches equilibrium with the gravitational load. This result validates the closed-loop control chain while also identifying motor torque margin as the main hardware-side limitation of the present configuration.
...
This repository is kept as a finalized engineering snapshot of the `demo_v6` firmware. The recommended reading and bring-up path is:
...
The core idea was to separate system-level design, interface contracts, and local implementation into different AI-assisted roles, while keeping human ownership over architecture, integration, hardware validation, and final engineering decisions.
...
My role was to define the system goals, maintain the source of truth, constrain the agents, review generated code, integrate modules, compile and flash firmware, observe real hardware failures, and feed hardware-grounded feedback back into the correct role.

This made the project an AI-native hardware-engineering workflow rather than a simple code-generation exercise. The final firmware remained grounded in compilation, flashing, serial observation, and physical prototype validation.
...
* The present motor / power / mechanical configuration has insufficient torque margin for full 45° holding under the current gravity-loaded folding-arm setup.
* RAM and FLASH usage are close to the STM32F103C8T6 limits, which leaves limited room for additional features without optimization or platform migration.
* The demo firmware validates the electrical-control chain but does not implement a full autonomous aerial manipulation workflow.
